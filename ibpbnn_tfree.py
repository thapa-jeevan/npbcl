...
class IBP_BAE(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, training_size, max_tasks,
                 ...
                 single_head=False, acts=None, extend=False):
        ...
        for t in range(self.num_tasks):
            ...
            for l in range(self.no_layers - 1):
                ...
                if (t < self.num_tasks - 1):
                    ...
                    _conc1 = nn.Parameter(self.softplus_inverse(self.constant(np.ones((dout)) * alpha[t][l])),
                                          requires_grad=rgrad)
                    _conc2 = nn.Parameter(self.softplus_inverse(self.constant(np.ones((dout)) * beta[t][l])),
                                          requires_grad=rgrad)
                else:
                    alphas_t.append(self.constant(alpha[t - 1][l]))  # Prior
                    betas_t.append(self.constant(beta[t - 1][l]))  # Prior
                    # Modified Variatonal Parameters contrained to be positive by taking inverse softplus then softplus.
                    _conc1 = nn.Parameter(self.softplus_inverse(self.constant(np.ones((dout)) * alpha[t - 1][l])),
                                          requires_grad=rgrad)
                    _conc2 = nn.Parameter(self.softplus_inverse(self.constant(np.ones((dout)) * beta[t - 1][l])),
                                          requires_grad=rgrad)
    ...
    def ibp_sample(self, l, no_samples, temp=0.1, tid=-1):
        ...
        vs = self.v_post_distr(l, shape=[no_samples, din],
                               tid=tid)  # Independently sampling current layer IBP posterior : K x din x dout
        pis = torch.cumprod(vs, dim=2)  # Calcuting Pi's using nu's (IBP prior log probabilities): K x din x dout
        method = 0
        if (method == 0):
            logit_post = self._p_bers[tid][l].unsqueeze(0) + self.logit(
                pis)  # Varaitonal posterior log_alpha: K x din x dout
        elif (method == 1):
            logit_post = self._p_bers[tid][l].unsqueeze(0) + torch.log(
    ...
    def def_cost(self, x, y, task_id, temp, fix=False):
        ...
        if (not fix):
            ...
            # print(self.cost2)
            # assert False
            self.cost3 = (self._KL_v(task_id) * self.global_multiplier + sum(self.KL_B) + sum(self.KL_G)).div(
    ...
    def prediction_prob(self, x_test, task_idx, batch_size=1000):
        ...
        with torch.no_grad():
            ...
            for i in range(total_batch):
                ...
                pred_const_mask, _ = self._prediction(batch_x, task_idx, self.no_pred_samples, const_mask=True,
                                                      temp=self.min_temp)
    ...
    def train_step_all(self, x, y, task_id, temp, fix=False):
        ...
        for tid in range(self.num_tasks):
            ...
            if (tid != self.num_tasks - 1):
                ...
                q_tid = torch.bmm(z, self.ws_cla[task_id].view(1, -1, 1).repeat(z.shape[0], 1, 1)) + self.bs_cla[
                    task_id]
        ...
        if (val_size != 0):
            ...
            for epoch in range(no_epochs):
                ...
                for i in range(total_batch):
                    ...
                    sys.stdout.write("\rEpoch :" + str(epoch) + " [" + "=" * int(i / increment) + " " * int(
                        (total_batch - i) / increment) + "]" + str(i * 100.0 / total_batch) + "%")
                    sys.stdout.flush()
